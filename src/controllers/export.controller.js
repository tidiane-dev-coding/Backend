...
exports.exportEmployeesPDF = async (req, res) => {
  try {
    const employees = await Employee.find().populate('service').sort({ nom: 1, prenom: 1 });
    ...
    res.setHeader('Content-Disposition', `attachment; filename=employees-directory-${Date.now()}.pdf`);

    console.log('--- DEBUT GENERATION PDF EMPLOYÉS ---');
    ...
    const drawHeader = () => {
      ...
      if (logoPath) {
        ...
        console.error('❌ Logo introuvable dans:', possiblePaths);
        // Fallback
        doc.circle(60, 60, 25).fill('#ffffff').stroke(secondaryColor, 2);
        doc.fontSize(20).fillColor(primaryColor).text('RH', 50, 50, { align: 'center', width: 20 });
      }

      // Nom de l'entreprise - TITRE CORRIGÉ
      doc.fontSize(20)
        .font('Helvetica-Bold')
        .fillColor('#ffffff')
        .text('CONSEIL GUINEEN DES CHARGEURS', 100, 35, { width: 400 });
    ...
    const drawFooter = () => {
      ...
      doc.moveTo(margin, footerY - 10).lineTo(pageWidth - margin, footerY - 10).stroke(borderColor, 1);
      doc.fontSize(9).fillColor(darkGray).text(`Page ${pageNumber}`, margin, footerY, { align: 'left' });
      doc.text('© CONSEIL GUINEEN DES CHARGEURS - Tous droits réservés', pageWidth - margin - 250, footerY, { width: 250, align: 'right' });
    };
    ...
    employees.forEach((emp, index) => {
      ...
      const empName = `${emp.nom} ${emp.prenom || ''}`.trim().substring(0, 30);
    ...
    employees.forEach((emp, index) => {
      ...
      doc.fontSize(22)
        .font('Helvetica-Bold')
        .fillColor(primaryColor)
        .text(`${emp.nom || '-'} ${emp.prenom || ''}`, margin + 15, y + 10);
      ...
      // Colonne 1
      let yPos = y;
      drawField('Sexe:', emp.genre, col1X, yPos);
      yPos += fieldHeight;
      drawField('Date de naissance:', emp.dateNaissance ? dayjs(emp.dateNaissance).format('DD/MM/YYYY') : '-', col1X, yPos);
      yPos += fieldHeight;
      drawField('Lieu de naissance:', emp.lieuNaissance || '-', col1X, yPos);
      yPos += fieldHeight;
      drawField('Âge:', age ? `${age} ans` : '-', col1X, yPos);
      yPos += fieldHeight;
      drawField('Nationalité:', emp.nationalite || '-', col1X, yPos);
      yPos += fieldHeight;
      drawField('Lieu de résidence:', emp.lieuResidence || '-', col1X, yPos);
      ...
      y += fieldHeight;
      drawField('Catégorie professionnelle:', emp.cadre || '-', col1X, y);
...
exports.exportAttendancePDF = async (req, res) => {
  try {
    ...
    const attendance = await Attendance.find(query)
      .populate('employee', 'nom prenom matricule poste')
      .sort({ date: -1 });
    ...
    const drawHeader = () => {
      ...
      if (logoPath) {
        try { doc.image(logoPath, 50, 20, { width: 80 }); } catch (e) { }
      } else {
        doc.circle(60, 60, 25).fill('#ffffff').stroke(secondaryColor, 2);
        doc.fontSize(20).fillColor(primaryColor).text('RH', 50, 50, { align: 'center', width: 20 });
      }
      doc.fontSize(20).font('Helvetica-Bold').fillColor('#ffffff').text('CONSEIL GUINEEN DES CHARGEURS', 100, 35, { width: 400 });
      doc.fontSize(11).font('Helvetica').fillColor('#e0e7ff').text('Registre des Présences', 100, 60);
      doc.fontSize(9).fillColor('#c7d2fe').text(`Généré le ${dayjs().format('DD/MM/YYYY HH:mm')}`, pageWidth - margin - 150, 35, { width: 150, align: 'right' });
    };

    const drawFooter = () => {
      doc.fontSize(9).fillColor(darkGray).text(`Page ${pageNumber}`, margin, pageHeight - 40);
    };

    drawHeader();

    let y = 150;
    doc.fontSize(18).font('Helvetica-Bold').fillColor(primaryColor).text('RAPPORT DE PRÉSENCES', margin, y, { align: 'center' });
    y += 30;
    ...
    doc.text('Nom & Prénom', startX + 150, y);
    ...
    attendance.forEach(att => {
      ...
      // Utiliser le nom de l'employé peuplé (avec prénom) si disponible, sinon le nom stocké dans attendance
      const name = att.employee ? `${att.employee.nom} ${att.employee.prenom || ''}`.trim() : att.nom;
      doc.text(name || '', startX + 150, y);
...
exports.exportServiceEmployeesPDF = async (req, res) => {
  try {
    ...
    // Fonction pour dessiner l'en-tête (Consolidée)
    const drawHeader = () => {
      doc.rect(0, 0, pageWidth, 120).fill(primaryColor);
    ...
    employees.forEach((emp, index) => {
      ...
      doc.fontSize(14)
        .font('Helvetica-Bold')
        .fillColor(primaryColor)
        .text(`${emp.nom || '-'} ${emp.prenom || ''}`, margin + 10, currentY);
      ...
      doc.font('Helvetica-Bold').fillColor(darkGray);
      doc.text('Catégorie professionnelle:', rightColumnX, rightY);
      doc.font('Helvetica').fillColor(darkGray);
...
exports.exportEmployeePDF = async (req, res) => {
  try {
    ...
    const fs = require('fs');
    const possiblePaths = [
      path.join(process.cwd(), 'backend', 'assets', 'cropped-logo (1).png'),
      path.join(process.cwd(), 'backend', 'assets', 'logo.png'),
      path.join(process.cwd(), 'assets', 'cropped-logo (1).png'),
      path.join(process.cwd(), 'assets', 'logo.png'),
      path.join(__dirname, '../../assets/cropped-logo (1).png'),
      path.join(__dirname, '../../assets/logo.png')
    ];
    ...
    if (logoPath) {
      console.log('✅ Logo trouvé pour fiche employé:', logoPath);
      try {
        const imgBuffer = fs.readFileSync(logoPath);
        doc.image(imgBuffer, 50, 30, { width: 80 });
      } catch (e) {
        console.error('❌ Erreur lors du chargement de l\'image pour le PDF:', e && e.stack ? e.stack : e);
      }
    } else {
      doc.circle(70, 70, 30).fill('#ffffff').stroke(secondaryColor, 2);
      doc.fontSize(24).fillColor(primaryColor).text('RH', 60, 60, { align: 'center', width: 20 });
    }

    doc.fontSize(22).font('Helvetica-Bold').fillColor('#ffffff')
      .text('CONSEIL GUINEEN DES CHARGEURS', 140, 40, { width: 400 });

    doc.fontSize(12).font('Helvetica').fillColor('#e0e7ff')
      .text('Fiche Individuelle de l\'Employé', 140, 75);

    const currentDate = dayjs().format('DD/MM/YYYY à HH:mm');
    doc.fontSize(9).fillColor('#c7d2fe')
      .text(`Généré le ${currentDate}`, pageWidth - margin - 150, 40, { width: 150, align: 'right' });

    doc.text(`Document confidentiel`, pageWidth - margin - 150, 55, { width: 150, align: 'right' });
    ...
    drawInfoField('Catégorie professionnelle:', employee.cadre || '-', rightColumnX, rightY, colWidth);
